...
//
// Section 30 covers the one thing 3.0 adds: the _cvo_aud audience cookie. Two
// jobs that are deliberately separate. It reads the cookie an earlier page view
// left and exposes it, which is what makes an audience available at time zero to
// anything in the head; and it refreshes that cookie after load, for the next
// page view, because a nightly-derived audience gains nothing from being fetched
// during this one. Sections 1 to 29 are the other half of the check: 3.0 is
// additive and none of them may move.
//
// Usage: node test/runtime-tag-3.0.test.js
'use strict';
...
var TAG_PATHS = [
  { label: 'conversio_runtime_tag_v3.0.js', path: path.join(__dirname, '..', 'conversio_runtime_tag_v3.0.js') },
  { label: 'self-hosted/public/runtime-tag.3.0.js', path: path.join(__dirname, '..', 'self-hosted', 'public', 'runtime-tag.3.0.js') }
];

// The slot the loader substitutes the client's audience endpoint into, and a
// value shaped like one it would really serve.
var AUDIENCE_SLOT = '@@CONVERSIO_AUDIENCE_ENDPOINT@@';
var ENDPOINT = 'https://tag.conversio.dev/a/cvo_0123456789abcdefghij/';
...
function runSuite(tagPath, label) {
  ...
  // Stands in for a loader serving a client who has audiences enabled.
  function tagWithAudiences(endpoint, opts) {
    opts = opts || {};
    opts.tagPath = tagPath;
    opts.tagSource = source.split(AUDIENCE_SLOT).join(endpoint);
    return runTag(opts);
  }
  ...
  (function () {
    ...
    check('only expected globals', JSON.stringify(added) === JSON.stringify([
      ...
      'conversioAbtastyQueue', 'conversioAbtastyTracking', 'conversioAudience',
      'conversioConsentQueue', 'conversioSettings'
  ...
  // 30. Audiences: the _cvo_aud cookie
  //
  // The one thing 3.0 adds, and it is two jobs that are deliberately separate.
  //
  // READ: the cookie is parsed at init and exposed, because it was written on an
  // earlier page view and is therefore already present. That is the whole design:
  // the client's experimentation platform reads the same cookie from the head at
  // time zero, long before this tag runs.
  //
  // WRITE: a refresh scheduled after load, for the NEXT page view. Audiences are
  // derived nightly, so fetching one during this page load would buy nothing.
  // Nothing here may block, delay or break anything, and every failure path has
  // to leave the existing cookie alone: a stale audience still targets, an
  // absent one does not.
  (function () {
    var HOUR = 3600;
    var now = Math.floor(Date.now() / 1000);
    var fresh = 'v1.' + (now - 60) + '.,lapsed_90d,high_aov,';
    var stale = 'v1.' + (now - 13 * HOUR) + '.,lapsed_90d,';

    function aud(res) { return res.window.conversioAudience; }
    function cookie(res) { return res.cookies._cvo_aud; }
    function lastWrite(res) {
      return res.cookieWrites.length ? res.cookieWrites[res.cookieWrites.length - 1] : '';
    }

    // --- reading what is already there ---
    var r = tag(consented({ cookies: { _cvo_aud: fresh } }));
    check('a cookie already on the visitor is exposed',
      !!aud(r) && aud(r).audiences.join(',') === 'lapsed_90d,high_aov', JSON.stringify(aud(r)));
    check('with the computed_at it carried', !!aud(r) && aud(r).ts === now - 60, JSON.stringify(aud(r)));
    check('and no request is made for a fresh one', r.xhrRequests.length === 0);
    ...
    [['a truncated value', 'v1.'],
     ...
     ['something else entirely under the same name', 'not-our-cookie'],
     ['an empty value', '']].forEach(function (pair) {
      check('treats ' + pair[0] + ' as absent rather than repairing it',
        aud(tag(consented({ cookies: { _cvo_aud: pair[1] } }))) === null, pair[1]);
    });

    r = tag(consented({ cookies: { _cvo_aud: 'v1.' + now + '.,good,BAD,has space,also_good,' } }));
    check('codes that fail the pattern are dropped on the way in',
      aud(r).audiences.join(',') === 'good,also_good', JSON.stringify(aud(r)));

    // --- nothing at all before consent ---
    r = tag({ cookies: { _cvo_aud: fresh } });
    check('an unconsented visitor gets no audience exposed', aud(r) === undefined || aud(r) === null);
    check('and no request is made', r.xhrRequests.length === 0);

    // --- refreshing, and when it does not ---
    check('an unconfigured endpoint makes no request at all',
      tag(consented({})).xhrRequests.length === 0);

    r = tagWithAudiences(ENDPOINT, consented({ xhr: { body: { v: 1, ts: now, a: ['lapsed_90d'] } } }));
    check('a visitor with no cookie is looked up', r.xhrRequests.length === 1,
      JSON.stringify(r.xhrRequests));
    check('at the endpoint the loader patched in, with the id appended',
      r.xhrRequests[0].url.indexOf(ENDPOINT) === 0 &&
      /con_[a-z2-7]{16}\.[0-9]+$/.test(r.xhrRequests[0].url), r.xhrRequests[0].url);
    check('as a plain GET, so the browser sends no preflight',
      r.xhrRequests[0].method === 'GET');
    check('with a timeout set, since nothing may hang',
      typeof r.xhrRequests[0].timeout === 'number' && r.xhrRequests[0].timeout > 0);

    check('a fresh cookie is not looked up again',
      tagWithAudiences(ENDPOINT, consented({ cookies: { _cvo_aud: fresh } })).xhrRequests.length === 0);
    check('a stale one is',
      tagWithAudiences(ENDPOINT, consented({ cookies: { _cvo_aud: stale } })).xhrRequests.length === 1);

    // Nothing on this page needs the answer, so it must not compete with the
    // page's own resources.
    r = tagWithAudiences(ENDPOINT, consented({
      autoDrain: false, readyState: 'loading', xhr: { body: { v: 1, ts: now, a: ['lapsed_90d'] } }
    }));
    check('the request is deferred rather than made during init', r.xhrRequests.length === 0);
    r.drain();
    check('and goes out once the page has loaded', r.xhrRequests.length === 1);

    // --- writing the cookie ---
    r = tagWithAudiences(ENDPOINT, consented({
      xhr: { body: { v: 1, ts: now, a: ['lapsed_90d', 'high_aov'] } }
    }));
    check('a good response writes the cookie',
      cookie(r) === 'v1.' + now + '.,lapsed_90d,high_aov,', cookie(r));
    check('wrapped in delimiters, so a target cannot match a longer code',
      cookie(r).indexOf('.,') !== -1 && cookie(r).slice(-1) === ',', cookie(r));
    check('and exposes it without waiting for the next page load',
      !!aud(r) && aud(r).audiences.join(',') === 'lapsed_90d,high_aov', JSON.stringify(aud(r)));

    var write = lastWrite(r);
    check('the cookie is path-scoped to the site', write.indexOf('Path=/') !== -1, write);
    check('and outlives the refresh window', /Max-Age=2592000/.test(write), write);
    check('and is SameSite=Lax', /SameSite=Lax/.test(write), write);
    check('and Secure on https', /Secure/.test(write), write);

    r = tagWithAudiences(ENDPOINT, consented({
      protocol: 'http:', xhr: { body: { v: 1, ts: now, a: ['lapsed_90d'] } }
    }));
    check('but not Secure on http, where it would be dropped silently',
      !/Secure/.test(lastWrite(r)), lastWrite(r));

    r = tagWithAudiences(ENDPOINT, consented({
      xhr: { body: { v: 1, ts: now, a: ['good', 'has,comma', 'BAD', 'has space', 'fine_2'] } }
    }));
    check('a code containing a comma never reaches the cookie',
      cookie(r).indexOf('has,comma') === -1 && cookie(r) === 'v1.' + now + '.,good,fine_2,', cookie(r));

    var many = [];
    for (var i = 0; i < 40; i++) many.push('code_' + i);
    r = tagWithAudiences(ENDPOINT, consented({ xhr: { body: { v: 1, ts: now, a: many } } }));
    check('the cookie cannot exceed its budget however many codes arrive',
      aud(r).audiences.length === 24, String(aud(r).audiences.length));
    check('and keeps the first 24 rather than an arbitrary 24',
      aud(r).audiences[0] === 'code_0' && aud(r).audiences[23] === 'code_23');

    r = tagWithAudiences(ENDPOINT, consented({ xhr: { body: { v: 1, ts: now, a: [] } } }));
    check('a visitor in no audience still gets a cookie, so they are not re-asked',
      cookie(r) === 'v1.' + now + '.,', cookie(r));

    // --- every failure leaves what was there alone ---
    [['a 500', { status: 500, body: { v: 1, ts: now, a: ['x'] } }],
     ['a 404', { status: 404, body: {} }],
     ['a network error', { networkError: true }],
     ['a timeout', { timeoutError: true }],
     ['an unparseable body', { body: 'not json' }],
     ['a payload from a future schema', { body: { v: 2, ts: now, a: ['x'] } }],
     ['a payload with no timestamp', { body: { v: 1, a: ['x'] } }],
     ['a payload whose a is not a list', { body: { v: 1, ts: now, a: 'lapsed_90d' } }]
    ].forEach(function (pair) {
      var res = tagWithAudiences(ENDPOINT, consented({
        cookies: { _cvo_aud: stale }, xhr: pair[1]
      }));
      check(pair[0] + ' leaves the existing cookie untouched',
        res.cookies._cvo_aud === stale, res.cookies._cvo_aud);
      check(pair[0] + ' still leaves the tag initialised',
        dataEvents(res.dataLayer).length === 1);
    });

    // --- cookies unavailable entirely ---
    r = tagWithAudiences(ENDPOINT, consented({
      cookiesThrow: true, xhr: { body: { v: 1, ts: now, a: ['lapsed_90d'] } }
    }));
    check('a browser blocking cookies does not break the tag',
      dataEvents(r.dataLayer).length === 1);
    // Worth stating as intended rather than tolerated: the cookie is how an
    // audience reaches the NEXT page view, so a browser that cannot store one
    // loses that, not the answer itself. This page still has it.
    check('and this page still gets the audience it fetched, ephemerally',
      !!aud(r) && aud(r).audiences.join(',') === 'lapsed_90d', JSON.stringify(aud(r)));
    check('it simply does not persist to the next page view', !cookie(r));

    // --- withdrawal ---
    r = tag(consented({ cookies: { _cvo_aud: fresh } }));
    check('the cookie is there to begin with', !!cookie(r));
    r.window.__conversioDisableEmission__();
    check('withdrawing consent deletes the audience cookie', !cookie(r), cookie(r));
    check('and expires it rather than blanking it', /Max-Age=0/.test(lastWrite(r)), lastWrite(r));
    check('and stops exposing it', aud(r) === null);

    // --- the rest of the tag is untouched ---
    r = tagWithAudiences(ENDPOINT, consented({ xhr: { body: { v: 1, ts: now, a: ['lapsed_90d'] } } }));
    check('conversio_data still fires exactly once', dataEvents(r.dataLayer).length === 1);
    check('and carries no audience, the cookie being where that lives',
      !('conversio_audience' in dataEvents(r.dataLayer)[0].conversio),
      JSON.stringify(dataEvents(r.dataLayer)[0].conversio));
  })();
