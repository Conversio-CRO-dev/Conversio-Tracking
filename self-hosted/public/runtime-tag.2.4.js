// CONVERSIO TAG | Runtime bundle : version 2.4 - updated 03-08-2026
...
(function () {
  ...
  // is preferred for its sub-millisecond resolution, but browsers clamp
  // performance.now() (typically to 100us) as a Spectre mitigation, so the
  // trailing digits are not truly microsecond-accurate. Uniqueness rests on
  // the random component, not on this clock.
  function nowMicroseconds() {
    ...
    try {
      if (perf && typeof perf.now === 'function' && typeof perf.timeOrigin === 'number') {
        ...
        if (isFinite(ms) && ms > 0) return Math.round(ms * 1000);
      }
    } catch (e) {
  ...
  // absent from most of these sends by nature: experiences and events fire when
  // they occur, which is usually well before Core Web Vitals collection has
